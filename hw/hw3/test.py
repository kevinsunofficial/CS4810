...
if __name__=='__main__':
    ...
    names = [file.split('.')[0] for file in os.listdir('./') if file.endswith('.txt') and file.startswith('hw3')] if option=='all' else [option]
    allinfo = []
    names = tqdm(names, leave=True)
    for name in names:
        names.set_description("Inspecting %s" % name)
        hw3.drawfile(name+'.txt')
            ...
            for y in range(height):
                ...
                if tp == mp:
                    ...
                    if tp != (0,0,0,0) and mp == (0,0,0,0):
                        ...
                        # print(np.array(list(tp[:3]))/255)
                        image.putpixel((x,y),(255,125,50,255))
                        tc, mc = '/'.join(np.array(tp[:3]).astype(str).tolist()), '/'.join(np.array(mp[:3]).astype(str).tolist())
                        info.append(', '.join([name,str(x),str(y),'Wrong Color', tc, mc])+'\n')
